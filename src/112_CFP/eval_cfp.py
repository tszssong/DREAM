...
def extract_feat(arch, resume):
    ...
    model = None
    assert(arch in ['resnet18','resnet50','resnet101','mobilefacenet'])
    if arch == 'resnet18':
    ...
    if arch == 'mobilefacenet':
        model = mobilefacenet(pretrained=False, num_classes=class_num,\
                extract_feature=True, end2end=end2end)
...
if __name__ == '__main__':
    ...
    infos = [ ('mobilefacenet_naive', '../112_e2e/model_naive/mobilefacenet_%d.pth.tar'%i) for i in range(1,80) ]
    #infos = [ ('resnet18_naive', '../112_e2e/model_naive/resnet18_%d.pth.tar'%i) for i in range(1,80) ]
